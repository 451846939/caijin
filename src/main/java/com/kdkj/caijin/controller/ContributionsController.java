...
//        String realPath = request.getSession().getServletContext().getRealPath("files/");
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "usersid", value = "用户id", dataType = "String", paramType = "query", required = false)
    })
    @GetMapping("/findByUser")
    public Result findByUser(String usersid,Pageinfo pageinfo){
        try {
            List<ContributionsVo> byUserid = contributionsService.findByUserid(usersid, PageUtis.getPageRequest(pageinfo, Sort.Direction.ASC));
            return Result.ok("成功",byUserid);
        } catch (Exception e) {
            return Result.error(e.getMessage());
        }
    }
